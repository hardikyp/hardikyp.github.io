(() => {
  ...
  if (!slug) {
    applyUnavailableDetailMetadata('updates');
    return;
  }
  ...
  const load = async () => {
    try {
      ...
      try {
        ...
        const itemTitle = u.title || 'Update';
        ...
        const imageAlt = u.image?.alt || itemTitle || 'Hardik Patil update';
        applyDetailPageMetadata({
          ...
          "breadcrumb": breadcrumbList('updates', itemTitle, canonicalUrl),
